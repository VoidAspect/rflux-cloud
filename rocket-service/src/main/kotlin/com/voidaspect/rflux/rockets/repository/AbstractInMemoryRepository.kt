...
abstract class AbstractInMemoryRepository<T, E : Identity<T, ID>, N : T, ID> : RepositoryOperations<T, E, N, ID> {
    ...
    override fun update(entity: E) = if (store.containsKey(entity.id)) {
    ...
    override fun add(entity: N) = initialize(entity)
            .also { store[it.id] = it }
            .let { Mono.just(it) }

    override operator fun get(id: ID) = Mono.justOrEmpty(store[id])

    override fun remove(id: ID) = Mono.justOrEmpty(store.remove(id))

    override fun findAll() = Flux.fromIterable(store.values)

    override fun contains(id: ID) = Mono.just(store.contains(id))
}